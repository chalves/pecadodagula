...
class Mural extends CI_Controller {
    ...
    function cadastrar() {
        if ($this->session->userdata('logged_in')) {
            ...
            $dados['opcaoView'] = 'Apoio/Mural';
            $dados ['telaativa'] = 'Mural/cadastrar';
            $dados['opcaoMenu'] = 'Mural';

            $dados ['idDE'] = $dadosLogin['idUsuario'];
            $dados ['idEmpresa'] = $dadosLogin['idEmpresa'];
            
            $dados ['assunto'] = $this->input->post('assunto-input');
            $dados ['destinatario'] = $this->input->post('destinatario-input');
            $dados ['prioridade'] = $this->input->post('prioridade-input');
            $dados ['recado'] = $this->input->post('recado-input');

            $this->load->model('model_mural');
            $resultadocadastromural = $this->model_mural->cadastrar($dados);

            if ($resultadocadastromural) {
                $this->load->model('model_mural');
                $resultadocadastromural = $this->model_mural->buscar();
