...
if ($this->session->userdata('logged_in')) {
    ...
    $dados['opcaoView'] = 'Mural - Respondidos';
    $dados ['telaativa'] = 'Apoio';
    $dados['opcaoMenu'] = 'Mural';
    $this->load->model('model_mural');
    $resultadoMural = $this->model_mural->buscarRespondidos();
    ...
    if (( isset($resultadoMural) ) && (!empty($resultadoMural) )) {
        foreach ($resultadoMural as $mural) {
            ...
            if ($mural->enviouResposta == '1') {
                $_SESSION['temResposta'] = '1';
            } else {
                if (isset($_SESSION['temResposta'])) {
                    unset($_SESSION['temResposta']);
                }
            }
            ?>                        
        ...
        <div class="col-md-4 ">
            <div class = "alert alert-success" role = "alert" style = "width: 350px; margin: 20 auto; padding-top: 10px;">
                Vocë existe nenhum Recado no Mural respondido...
            </div>        
        </div>
        <?php
    }
